...
class TestAfterEffectsRPC(TestAdobeRPC):
    ...
    def tearDown(self):
        self.engine.adobe.app.project.close(
            self.engine.adobe.CloseOptions.DO_NOT_SAVE_CHANGES
        )
    ...
    def test_is_footage_item(self):
        ...
        found_items = []
        for itm in self.engine.iter_collection(self.engine.adobe.app.project.items):
            if self.engine.is_item_of_type(
                itm, self.engine.AdobeItemTypes.FOOTAGE_ITEM
            ):
